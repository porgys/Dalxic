...
export function HealthShell({ children }: { children: ReactNode }) {
...
function Header({ orgName, operatorName, onLogout }: { orgName: string; operatorName: string; onLogout: () => void }) {
  return (
    <div style={{
      ...
      <Link href="/health" style={{ display: "flex", alignItems: "center", gap: 12, textDecoration: "none" }}>
        <HealthBrand />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{
          ...
          background: `${HEALTH_COL}08`, border: `1px solid ${T.border}`,
        }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: HEALTH_COL, display: "inline-block", animation: "pulse 2s infinite" }} />
          <span style={{ fontSize: 11, fontWeight: 600, color: HEALTH_COL }}>Online</span>
        </div>
...
export function HealthBrand({ size = 14 }: { size?: number }) {
  return (
    <span style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
      ...
      <span style={{
        ...
        background: `linear-gradient(135deg, ${HEALTH_COL}, #E69A2E)`,
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
        <div style={{ position: "absolute", top: "20%", left: "10%", width: 480, height: 480, borderRadius: "50%", background: "radial-gradient(circle, rgba(217,119,6,0.10), transparent 70%)", filter: "blur(40px)" }} />
