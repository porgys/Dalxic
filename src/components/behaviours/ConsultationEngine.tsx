...
import { useVoiceCallout, VoiceCalloutOverlay, MuteToggle } from "@/components/workspace/VoiceCallout"
...
function SOAPView({ ax, accent, label }: { ax: string; accent: Accent; label: string }) {
  ...
  const [noteTab, setNoteTab] = useState<"notes" | "vitals" | "rx" | "lab" | "refer">("notes")
  ...
  const [vitals, setVitals] = useState({ hr: "", bp: "", temp: "", spo2: "", rr: "", weight: "" })
      ...
      <div style={{ ...glass }}>
        {patient ? (
          <>
            ...
            <Tabs tabs={[{ key: "notes" as const, label: "SOAP Notes" }, { key: "vitals" as const, label: "Vitals" }, { key: "rx" as const, label: "Prescriptions", count: rxList.length }, { key: "lab" as const, label: "Lab Orders", count: labList.length }, { key: "refer" as const, label: "Referrals" }]} value={noteTab} onChange={setNoteTab} />
            ...
            {noteTab === "vitals" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
                  <div><label style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: ax, display: "block", marginBottom: 6, fontFamily: "'DM Mono', monospace" }}>Heart Rate</label><input value={vitals.hr} onChange={e => setVitals(v => ({ ...v, hr: e.target.value }))} style={inputStyle(ax)} placeholder="bpm" /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: ax, display: "block", marginBottom: 6, fontFamily: "'DM Mono', monospace" }}>Blood Pressure</label><input value={vitals.bp} onChange={e => setVitals(v => ({ ...v, bp: e.target.value }))} style={inputStyle(ax)} placeholder="e.g. 120/80" /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: ax, display: "block", marginBottom: 6, fontFamily: "'DM Mono', monospace" }}>Temperature</label><input value={vitals.temp} onChange={e => setVitals(v => ({ ...v, temp: e.target.value }))} style={inputStyle(ax)} placeholder="°C" /></div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
                  <div><label style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: ax, display: "block", marginBottom: 6, fontFamily: "'DM Mono', monospace" }}>SpO2</label><input value={vitals.spo2} onChange={e => setVitals(v => ({ ...v, spo2: e.target.value }))} style={inputStyle(ax)} placeholder="%" /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: ax, display: "block", marginBottom: 6, fontFamily: "'DM Mono', monospace" }}>Resp Rate</label><input value={vitals.rr} onChange={e => setVitals(v => ({ ...v, rr: e.target.value }))} style={inputStyle(ax)} placeholder="breaths/min" /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: ax, display: "block", marginBottom: 6, fontFamily: "'DM Mono', monospace" }}>Weight</label><input value={vitals.weight} onChange={e => setVitals(v => ({ ...v, weight: e.target.value }))} style={inputStyle(ax)} placeholder="kg" /></div>
                </div>
                <Button variant="ghost" icon="check" onClick={() => setSaved(true)}>Record Vitals</Button>
                {saved && <div style={{ padding: "8px 14px", borderRadius: 8, background: `${T.emerald}14`, color: T.emerald, fontSize: 12, fontWeight: 600 }}>Vitals recorded</div>}
              </div>
            )}
...
function DisplayView({ ax, accent }: { ax: string; accent: Accent }) {
  ...
  const { callout, muted, toggleMute } = useVoiceCallout()
  const [callingId, setCallingId] = useState<string | null>(null)
  const [activeCallout, setActiveCallout] = useState<{ token: string; name: string; destination: string } | null>(null)
  ...
  function callPatient(q: QueueEntry) {
    setCallingId(q.id)
    setActiveCallout({ token: q.token, name: q.name, destination: "doctor" })
    callout({ id: q.id, token: q.token, name: q.name, destination: "doctor" })
    setTimeout(() => setCallingId(null), 6000)
  }

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <MuteToggle muted={muted} onToggle={toggleMute} accent={ax} />
      </div>

      {activeCallout && (
        <VoiceCalloutOverlay token={activeCallout.token} name={activeCallout.name} destination={activeCallout.destination} onDone={() => setActiveCallout(null)} />
      )}
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", color: T.txD, fontFamily: "'DM Mono', monospace" }}>Waiting ({waiting.length})</div>
          {waiting.length > 0 && (
            <button onClick={() => callPatient(waiting[0])} disabled={!!callingId} style={{
              padding: "6px 14px", borderRadius: 8, fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase",
              background: callingId ? `${ax}08` : `${ax}14`, border: `1px solid ${ax}28`, color: ax,
              cursor: callingId ? "wait" : "pointer", fontFamily: "'DM Mono', monospace", opacity: callingId ? 0.5 : 1,
            }}>
              {callingId ? "Calling..." : "Call Next"}
            </button>
          )}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12 }}>
          {waiting.map(q => (
            <div key={q.id} onClick={() => callPatient(q)} style={{ ...glass, padding: 16, cursor: callingId ? "default" : "pointer", border: callingId === q.id ? `2px solid ${ax}` : `1px solid ${T.border}` }}>
