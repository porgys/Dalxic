...
export async function POST(request: Request) {
  ...
  try {
    ...
    await logAudit({ orgId: auth.orgId, actorId: auth.operatorId, actorName: operator?.name ?? auth.role, action: "create_pay_run", entity: "pay_run", entityId: run.id, after: { period: data.period, totalGross, totalNet, employeeCount: employees.length } })
    return ok(run)
  } catch {
    return fail("An error occurred", 500)
  }
