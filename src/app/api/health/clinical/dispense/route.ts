...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (data.outOfStock) {
      ...
      await logAudit({ orgId: auth.orgId, actorId: auth.operatorId, actorName, action: "dispense_out_of_stock", entity: "clinical_record", entityId: record.id, after: { drug: data.drug } })
      return ok(record)
    }
    ...
    await logAudit({ orgId: auth.orgId, actorId: auth.operatorId, actorName, action: "dispense_drug", entity: "clinical_record", entityId: record.id, after: { drug: data.drug, quantity: data.quantity, batchNo: data.batchNo } })
    return ok(record)
  } catch {
