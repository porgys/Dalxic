...
const URGENCY_COLORS: Record<string, { bg: string; text: string }> = {
  stat: { bg: "rgba(239,68,68,0.08)", text: "#EF4444" },
  urgent: { bg: "rgba(245,158,11,0.08)", text: "#F59E0B" },
  routine: { bg: "rgba(34,197,94,0.08)", text: "#22C55E" },
};
...
// Available exam types — referenced by exam form dropdown
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const CT_EXAMS = [
  "CT Head", "CT Chest", "CT Abdomen", "CT Pelvis", "CT Spine", "CT Angiography",
  "X-Ray Chest", "X-Ray Extremity", "X-Ray Abdomen", "X-Ray Spine", "Fluoroscopy", "Mammography",
];
...
function RadiologyContent({ operator }: { operator: OperatorSession }) {
  ...
  const submitReport = async () => {
    ...
    try {
      await fetch("/api/imaging", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ hospitalCode: HOSPITAL_CODE, action: "report", recordId: reportingOrder.recordId, imagingId: reportingOrder.id, findings: reportForm.findings, impression: reportForm.impression, reportedBy: "radiologist" }) });
          ...
          <motion.div key={activeNav} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <WorkshopBox title={`Report — ${reportingOrder.examType}`} icon="📝" className="mb-4">
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <motion.button type="button" onClick={submitReport} disabled={!!processingId} whileHover={{ y: -1 }} whileTap={{ scale: 0.97 }}
                    style={{ flex: 1, padding: "12px 24px", borderRadius: 12, fontSize: 12, fontWeight: 700, color: "white", background: `linear-gradient(135deg, ${COPPER}, #D4956B)`, border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.5px" }}>
                    Submit Report
                  </motion.button>
            ...
            {filteredOrders.length === 0 && !reportingOrder ? (
              <WorkshopBox title="No Orders" icon="🫁">
                <p style={{ fontSize: 13, color: "#64748B", textAlign: "center", padding: 24 }}>No {activeNav === "completed" ? "Completed" : "Pending"} Imaging Orders.</p>
              </WorkshopBox>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {filteredOrders.map((order, i) => {
                  const urgencyInfo = URGENCY_COLORS[order.urgency] || URGENCY_COLORS.routine;
                  ...
                  return (
                    <motion.div key={order.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                      style={{ padding: 20, borderRadius: 14, background: theme.cardBg, border: `1px solid ${order.urgency === "stat" ? "rgba(239,68,68,0.15)" : "rgba(184,115,51,0.1)"}`, backdropFilter: "blur(12px)" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <span style={{ fontSize: 14, fontWeight: 800, fontFamily: "var(--font-jetbrains-mono), monospace", color: COPPER }}>{order.queueToken}</span>
                          ...
                          <span style={{ padding: "2px 8px", borderRadius: 6, fontSize: 9, fontWeight: 700, textTransform: "uppercase", background: urgencyInfo.bg, color: urgencyInfo.text }}>{order.urgency}</span>
                        </div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          {order.status === "pending" && (
                            <motion.button type="button" onClick={() => startScan(order)} disabled={processingId === order.id} whileHover={{ y: -1 }} whileTap={{ scale: 0.97 }}
                          ...
                          {order.status === "in_progress" && (
                            <motion.button type="button" onClick={() => setReportingOrder(order)} whileHover={{ y: -1 }} whileTap={{ scale: 0.97 }}
